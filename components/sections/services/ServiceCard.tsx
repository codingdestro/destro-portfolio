...
const ServiceCard = ({ style, title, subTitle, price, points }: Props) => (
  ...
  >
    <div className="text-center">
      <h2 className={`font-Georgia text-[44px] font-semibold`}>{title}</h2>
      <h4 className="text-[12px] text-gray-600 ">{subTitle}</h4>
    </div>
    <div>
      <h1 className={`font-Georgia text-[40px] font-bold`}>${price}/hour</h1>
    </div>

    <ServicePoints points={points} />

    <Button className="w-[14rem]  rounded-none">Hire me</Button>
  </div>
);
